...
def derive(P, E):
    ...
    Returns P'
    '''
    return set([ei for ei, e in enumerate(E) if P.issubset(e)])

def close_intent(X, g_prime, m_prime):
    '''
    Returns X''
    '''
    Xp = derive(X, g_prime)
    if not bool(Xp):
        return set(range(len(m_prime)))#.keys())
    return set(reduce(set.intersection, [g_prime[g] for g in Xp]))
